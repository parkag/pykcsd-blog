...
* managing big datasets - some matrices should be read from HDD and not kept in RAM all the time
	* check np.memmap and hdf5 data format
* investigate how much speedup can be achieved
	* especially: fix the n-dimensional integrals (numpy warns they don't converge fast enough)
